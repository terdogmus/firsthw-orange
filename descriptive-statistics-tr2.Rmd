---
  title: "Tanimlayici Istatistik"
author: "Tugcan Erdogmus"
date: "31.10.2021"
output: 
  html_document:
  css: styles.css
...
**Yüzde deger (Percentile)**
```{r}
quantile(veri$portakal, c(.32, .57, .98))
```

**Çeyrek deger (Quartile)**
```{r}
quantile(veri$portakal)
```

**Degisim araligi (Range)**
```{r}
range(veri$portakal)
...
**Çeyrekler arasi degisim araligi (Interquartile range)**
```{r}
IQR(veri$portakal)
```
...
**Degiskenlik katsayisi (Coefficient of variation)**
```{r}
# Create the function.
getdk <- function(d) {
  k <- 100*(sd(d)/mean(d))
...
summary(df)
ggplot(df, aes(x=agirlik)) + geom_histogram(binwidth=100)
```
